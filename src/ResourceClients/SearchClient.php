...
class SearchClient extends BaseClient
{
    ...
    public function fundraiser($searchTerms, $charityId, $pageSize = self::DEFAULT_PAGE_SIZE, $pageNumber = self::START_PAGE)
    {
        return $this->get("fundraising/search?q=" . urlencode($searchTerms) . "&PageSize=" . $pageSize . "&page=" . $pageNumber . "&charityId=" . $charityId);
    }

    public function inMemory(SearchInMemoryRequest $searchRequest, $pageSize = self::DEFAULT_PAGE_SIZE, $pageNumber = self::START_PAGE)
    {
        return $this->get("remember/search?" . http_build_query($searchRequest->getAttributes()) . "&PageSize=" . $pageSize . "&page=" . $pageNumber);
    }

    public function team(SearchTeamRequest $searchRequest, $pageSize = self::DEFAULT_PAGE_SIZE, $pageNumber = self::START_PAGE)
    {
